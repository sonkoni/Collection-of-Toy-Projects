...
@interface ViewControllerC () <MGUNumKeyboardDelegate, UITextFieldDelegate>
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [UIColor mgrColorWithLightModeColor:[UIColor mgrColorFromHexString:@"D5D7DD"]
                                                      darkModeColor:[UIColor mgrColorFromHexString:@"151515"]
    ...
    MGUNumKeyboard *keyboard = [[MGUNumKeyboard alloc] initWithFrame:CGRectZero
                                                              locale:nil
                                                          layoutType:MGUNumKeyboardLayoutTypeStandard2
                                                       configuration:[MGUNumKeyboardConfiguration forgeConfiguration]];

    keyboard.specialKeyHandlerBlock = ^{
        NSLog(@"specialKeyHandlerBlock !!!! 반응했다.");
    };
    keyboard.delegate = self;
    keyboard.allowsDoneButton = YES; //! MGUNumKeyboardLayoutTypeStandard2에서는 아무런 효과가 없다.
