...
class TableViewDiffableDataSource: UITableViewDiffableDataSource<String, Item> {
    ...
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        let sectionDic = allItems[section]
        return sectionDic.keys.first  // 어차피 하나밖에 없다.
    }
}
...
class MainTableViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "DropdownButton & DropSegControl"
        configureTableView()
        configureDataSource()
...
extension MainTableViewController  {
    ...
    func updateUI(animated: Bool = true) {
        ...
        for i in 0..<allItems.count {
            ...
            let sectionTitle : String = section.keys.first!  // 어차피 하나밖에 없다.
            let items = section.values.first! // 어차피 하나밖에 없다.
...
extension MainTableViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 0 {
                viewController = ViewController0.init();
            }
        } else if indexPath.section == 1 {
            if indexPath.row == 0 {
                viewController = ViewController1.init();
            }
        }
