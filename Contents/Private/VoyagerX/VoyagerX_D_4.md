...
- **옵셔널(Optional) 타입**: 변수에 값이 없을 수 있는 상황을 명확히 표현할 수 있도록 옵셔널 타입이 도입되었다. 옵셔널은 `nil` 값을 허용하며, 옵셔널 바인딩(`if let`, `guard let`)을 통해 안전하게 값을 처리할 수 있다. Objective-C에서는 리시버가 `nil`이면 메서드가 아예 실행이 안된다.
...
- **강력한 타입 시스템**: Swift는 정적 타입 언어로, 컴파일 시 타입 검사를 수행하여 타입 관련 오류(특히 제네릭)를 사전에 방지한다. 그렇지만 그것때문에 Objective-C로 코드를 작성할 때보다 XCode가 더 버벅댄다. SwiftUI를 사용하면 맥북에서 비행기 이륙하는 소리가난다.
...
- **프로토콜 지향 프로그래밍(Protocol-Oriented Programming)**: Swift는 프로토콜을 통해 다형성을 제공하며, 클래스뿐만 아니라 구조체와 열거형에서도 프로토콜을 사용할 수 있다. 프로토콜에 구현체가 존재할 수 있다는 점이 강력한 점으로 사료된다. 그것 때문에 난해해지는 면도 있다. 프로토콜의 구현체가 존재할 수 있으므로 약간 다중상속 비슷한 느낌이다. **Objective-C**에서도 프로토콜이 존재하지만, **클래스에서만 사용가능하고 구현부분을 제공할 수 없고 오직 인터페이스 제공만 가능하다**. 
...
Swift는 안전성, 간결한 문법, 성능, 현대적인 언어 기능을 갖춘 강력한 프로그래밍 언어이고 Apple 생태계 전반에서 애플리케이션을 개발하는 데 최적화되어 있다. Swift는 2015년 오픈 소스로 공개되어 Windows, Linux 등에서도 사용할 수 있지만, Apple 생태계 이외에는 거의 사용되지 않는다.
