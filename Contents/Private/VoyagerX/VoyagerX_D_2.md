...
- **V와 C의 접합**: 실제로 코드를 작성해보면 V와 C가 접합되어버리는 현상이 발생한다. 특히 `UIViewController`는 View의 확장이라고 볼 수 있기 때문에 그런현상이 심하게 나타난다.
- **View의 비대**: V와 C의 접합현상으로 인하여 뷰가 비대해지는 결과가 발생한다.
...
MVC 패턴은 애플리케이션의 구조를 명확히 분리하여 유지보수성과 확장성을 높이는 데 기여하기 위해 만들어졌다. 그러나 실제 코드 작성에서는 MVC 패턴은 V와 C의 접합현상 때문에 이를 개선한 MVVM 패턴이 애플리케이션 아키텍처의 기초로 널리 사용되되고 있다.
