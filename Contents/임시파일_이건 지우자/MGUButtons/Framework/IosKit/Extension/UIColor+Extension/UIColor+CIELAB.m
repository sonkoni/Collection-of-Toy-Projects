...
#define REFX_O2_D65 95.047 // 95.0489
#define REFY_O2_D65 100.0
#define REFZ_O2_D65 108.883 // 108.8840
...
#pragma mark - Public
+ (UIColor *)mgrColorWithLightness:(CGFloat)lightness A:(CGFloat)A B:(CGFloat)B alpha:(CGFloat)alpha {
    CGFloat x,y,z;
...
- (BOOL)mgrGetLightness:(CGFloat *)lightnessOut A:(CGFloat *)aOut B:(CGFloat *)bOut alpha:(CGFloat *)alphaOut {
    CGFloat R,G,B,alphaIn;
    if ([self getRed:&R green:&G blue:&B alpha:&alphaIn] == NO) {
        return NO;
    }
...
- (UIColor *)mgrOffsetWithLightness:(CGFloat)lightnessOffset a:(CGFloat)aOffset b:(CGFloat)bOffset alpha:(CGFloat)alphaOffset {
    CGFloat l = 0.0, a = 0.0, b = 0.0, alpha = 0.0;
    [self mgrGetLightness:&l A:&a B:&b alpha:&alpha];
    ...
    return [UIColor mgrColorWithLightness:l A:a B:b alpha:alpha];
}


#pragma mark - Public intermediate XYZ colorspace methods (mostly for tests)
- (void)mgrGetX:(CGFloat *)xOut Y:(CGFloat *)yOut Z:(CGFloat *)zOut alpha:(CGFloat *)alphaOut {
    CGFloat r,g,b,alpha;
