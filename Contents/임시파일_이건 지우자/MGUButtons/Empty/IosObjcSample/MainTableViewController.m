...
- (UIStatusBarStyle)preferredStatusBarStyle {
    ...
    /**
     UIStatusBarStyleDefault       밝은 배경에 사용하기위한 어두운 상태 막대. status bar 뒤쪽이 밝을 때 사용하고, 글자가 검은색으로 나옴.
     UIStatusBarStyleLightContent  어두운 배경에 사용하기위한 밝은 상태 막대. status bar 뒤쪽이 어두울 때 사용하고, 글자가 흰색으로 나옴.
     */
}

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"각종 버튼에 대하여 알아보자.";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        if(indexPath.row == 0){
            ...
            viewController = [ViewController2A new];
            viewController.navigationItem.title = [self.dataSource tableView:self.tableView titleForHeaderInSection:indexPath.section];
        }
    }
    ...
    if (indexPath.section == 2) {
        if(indexPath.row == 0){
            viewController = [ViewController4 new];
            viewController.navigationItem.title = [self.dataSource tableView:self.tableView titleForHeaderInSection:indexPath.section];
        }
    }
    ...
    if (indexPath.section == 5) {
        if(indexPath.row == 0){
            viewController = [ViewController7 new];
            viewController.navigationItem.title = [self.dataSource tableView:self.tableView titleForHeaderInSection:indexPath.section];
        }
    }
