...
#import "MGUCarouselCell.h"

static void *MGUCarouselCellKVOContext = &MGUCarouselCellKVOContext;

@interface MGUCarouselCell ()
@property (nonatomic, strong, nullable) UIView *selectedForegroundView; // 셀렉션 또는 하이라이트 되었을 때의 색을 살짝 보여주기 위해 존재함.
@end

@implementation MGUCarouselCell
...
- (void)dealloc {
    if (_textLabel) {
        [_textLabel removeObserver:self forKeyPath:@"font" context:MGUCarouselCellKVOContext];
    }
}
...
#pragma mark - 생성 & 소멸
static void CommonInit(MGUCarouselCell *self) {
    ...
    if (_textLabel == nil) {
        ...
        [_textLabel addObserver:self // 폰트가 바뀌면, 레이아웃을 갱신하여 크기를 맞춘다.
                     forKeyPath:@"font"
                        ...
                        context:MGUCarouselCellKVOContext];
    }
    return _textLabel;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == MGUCarouselCellKVOContext) {
