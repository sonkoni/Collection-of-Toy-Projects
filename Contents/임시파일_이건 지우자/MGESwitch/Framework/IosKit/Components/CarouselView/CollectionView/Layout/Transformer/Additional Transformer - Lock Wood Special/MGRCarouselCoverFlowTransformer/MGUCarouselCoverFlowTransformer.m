//
//  MGUCarouselCoverFlowTransformer.m
//  Copyright © 2022 Mulgrim Co. All rights reserved.
//

@import GraphicsKit;
#import "MGUCarouselCoverFlowTransformer.h"
#import "MGUCarouselCellLayoutAttributes.h"
#import "MGUCarouselView.h"
#import "MGUCarouselView_Internal.h"


#pragma mark - MGUCarouselCoverFlowTransformer
@interface MGUCarouselCoverFlowTransformer ()
...
@implementation MGUCarouselCoverFlowTransformer
...
//! MGUCarouselCollectionViewLayout에서 이 값을 사용하지만, MGUCarouselCoverFlowTransformer에서는 큰 의미가 없다.
...
- (void)applyTransformTo:(MGUCarouselCellLayoutAttributes *)attributes {
    ...
    if (_useRubberEffect == YES) { //! 이 transformer type에서 rubber effect를 사용할 것인가.
        if (self.carouselView.currentIndex != currentIndex) {
            if (self.carouselView.volumeType != MGUCarouselVolumeTypeInfinite) { // 유한!
                if (self.carouselView.currentIndex > currentIndex) {
                ...
                CGFloat offset;
                if (scrollDirection == MGUCarouselScrollDirectionHorizontal) {
                    offset = self.carouselView.collectionView.contentOffset.x;
                } else {
                    offset = self.carouselView.collectionView.contentOffset.y;
                }
        ...
        if (scrollDirection == MGUCarouselScrollDirectionHorizontal) {
            _currentOffset = self.carouselView.collectionView.contentOffset.x;
        } else {
            _currentOffset = self.carouselView.collectionView.contentOffset.y;
        }
...
//                    position = 0.0;
    ...
    attributes.center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    ...
    //attributes.zIndex = 100 - (NSInteger)(ABS(attributes.position));
...
- (void)carouselViewWillBeginDragging:(MGUCarouselView *)carouselView {
    ...
    if (_useRubberEffect == YES) {
        if (self.carouselView.scrollDirection == MGUCarouselScrollDirectionHorizontal) {
            _currentOffset = carouselView.collectionView.contentOffset.x;
        } else {
            _currentOffset = carouselView.collectionView.contentOffset.y;
        }
    }
}
