//
//  MGUCarouselDiceTransformer.m
//  Copyright © 2022 Mulgrim Co. All rights reserved.
//

@import BaseKit;
@import GraphicsKit;
#import "MGUCarouselDiceTransformer.h"
...
#pragma mark - MGUCarouselDiceTransformer
@interface MGUCarouselDiceTransformer ()
...
@implementation MGUCarouselDiceTransformer
...
- (void)applyTransformTo:(MGUCarouselCellLayoutAttributes *)attributes {
    ...
    //! 설명은 MGUCarouselCylinderTransformer.graffle 참고하라. 회전 시 화면을 넘치지 않게 중심축을 적당히 밀었다가 당겨준다.
...
//    CGFloat centralAxis = -radius; 밀어
    ...
    if (scrollDirection == MGUCarouselScrollDirectionHorizontal) {
        if (attributes.representedElementCategory == UICollectionElementCategoryCell) {
            ...
            if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceBack]) {
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, centralAxis);
...
//                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, radius + 0.01);
            ...
            } else if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceSideA]) { // top
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, centralAxis);
                ...
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, radius + 0.01);
            } else if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceSideB]) { // bottom
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, centralAxis);
                transform3D = CATransform3DRotate(transform3D, M_PI_2, -1.0, 0.0, 0.0);
                transform3D = CATransform3DRotate(transform3D, theta+M_PI_2, 0.0, 0.0, 1.0);
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, radius + 0.01);
            } else if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceFloating]) { // floating 미정...
            }
        }
...
//                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, -1 *(radius + 0.01));
            ...
            } else if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceSideA]) {
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, centralAxis);
                transform3D = CATransform3DRotate(transform3D, M_PI_2, 0.0, -1.0, 0.0);
                transform3D = CATransform3DRotate(transform3D, theta+ M_PI_2, 0.0, 0.0, 1.0);
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, radius + 0.01);
            } else if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceSideB]) {
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, centralAxis);
                transform3D = CATransform3DRotate(transform3D, M_PI_2, 0.0, 1.0, 0.0);
                transform3D = CATransform3DRotate(transform3D, theta + M_PI_2, 0.0, 0.0, -1.0);
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, radius + 0.01);
            } else if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceFloating]) {
                transform3D = CATransform3DTranslate(transform3D, 0.0, 0.0, centralAxis);
                transform3D = CATransform3DRotate(transform3D, theta, -1.0, 0.0, 0.0);
...
//                CGFloat constant = 0.0; // 이걸 변화시킴. // 눞힘.
                 ...
                 */
            } else if ([attributes.representedElementKind isEqualToString:MGUCarouselElementKindDiceFloatingBar]) {
...
+ (instancetype)diceTransformerWithDepth:(CGFloat)depth {
    return [MGUCarouselDiceTransformer diceTransformerWithInverted:NO depth:depth];
}
...
- (void)setItemSize:(CGSize)itemSize {
    ...
    //! 설명은 MGUCarouselCylinderTransformer.graffle 참고하라.
    //! 4차 방정식으로 구한다.
    CGFloat radius = (self.carouselView.scrollDirection == MGUCarouselScrollDirectionHorizontal) ? itemSize.width / 2.0 : itemSize.height / 2.0;
...
- (void)setFloatingType:(MGUCarouselDiceFloatingType)floatingType {
    [self setFloatingType:floatingType animated:NO];
}

- (MGUCarouselDiceFloatingType)floatingType {
    if (_toggle == 1.0) {
        return MGUCarouselDiceFloatingTypeUp;
    } else {
        return MGUCarouselDiceFloatingTypeDown;
    }
}
...
- (void)setFloatingType:(MGUCarouselDiceFloatingType)floatingType animated:(BOOL)animated {
    if (animated == NO) {
        if (floatingType == MGUCarouselDiceFloatingTypeUp) {
            _toggle = 1.0;
        } else {
            _toggle = 0.0;
        }
