...
@interface MMTTopButtonSkin () <CAAnimationDelegate>
...
@property (nonatomic, strong) UIImageSymbolConfiguration *symbolConfig;
@property (nonatomic, strong) UIImage *plusImage;
@property (nonatomic, strong) UIImage *questionmarkImage;
@end

@implementation MMTTopButtonSkin
...
- (void)commonInit {
    ...
    _duration = 0.1;
    _skinSymbol = MMTTopButtonSkinSymbolNone;
    [self setupOutContainer];
    [self setupCenterContainer];
    [self setupPlusImageView];
}
...
- (void)setupPlusImageView {
    ...
    [self.imageView.widthAnchor constraintEqualToAnchor:self.widthAnchor multiplier:0.5].active = YES;
    self.imageView.contentMode = UIViewContentModeScaleAspectFit;
    _symbolConfig = [UIImageSymbolConfiguration configurationWithWeight:UIImageSymbolWeightBold];
    _plusImage = [UIImage systemImageNamed:@"plus" withConfiguration:self.symbolConfig];
    _questionmarkImage = [UIImage systemImageNamed:@"questionmark" withConfiguration:self.symbolConfig];
    self.imageView.tintColor = [UIColor darkGrayColor];
    self.imageView.userInteractionEnabled = NO;
    self.imageView.alpha = 0.0;
}

#pragma mark - 세터 & 게터
- (void)setSkinSymbol:(MMTTopButtonSkinSymbol)skinSymbol {
    [self setSkinSymbol:skinSymbol completion:nil];
}


#pragma mark - Action
- (void)setSkinSymbol:(MMTTopButtonSkinSymbol)skinSymbol completion:(void(^_Nullable)(void))completionBlock {
    if (_skinSymbol == skinSymbol) {return;} // 변화가 없다면 그냥 나가라.
    
    if (_skinSymbol == MMTTopButtonSkinSymbolNone || skinSymbol == MMTTopButtonSkinSymbolNone) { // 어느 한쪽이 안보이는 상태라면.
        if (_skinSymbol == MMTTopButtonSkinSymbolNone) { // 어떤 이미지가 나타나야한다. 기존 상태가 alpha가 0.0인 상태이다.
            self.imageView.image = (skinSymbol == MMTTopButtonSkinSymbolPlus) ? self.plusImage : self.questionmarkImage;
        }
        void (^animations)(void) = ^{ self.imageView.alpha = (skinSymbol == MMTTopButtonSkinSymbolNone) ? 0.0 : 1.0;};
    ...
    } else { // 그림이 존재하는 상태에서 그림이 존재하는 상태로 변화한다.
        
        void (^completion)(UIViewAnimatingPosition) = ^(UIViewAnimatingPosition finalPosition) {
            if (skinSymbol == MMTTopButtonSkinSymbolPlus) {
                self.imageView.image = self.plusImage;
            } else {
                self.imageView.image = self.questionmarkImage;
            }
            [UIViewPropertyAnimator runningPropertyAnimatorWithDuration:0.15
                                                                  delay:0.0
                                                                options:UIViewAnimationOptionCurveEaseInOut
        ...
        [UIViewPropertyAnimator runningPropertyAnimatorWithDuration:0.15
                                                              delay:0.0
                                                            options:UIViewAnimationOptionCurveEaseInOut
                                                         animations:^{self.imageView.alpha = 0.0;}
                                                         completion:completion];
    }
    _skinSymbol = skinSymbol;
...
- (void)setSkinType:(MGUOnOffSkinType)skinType animated:(BOOL)animated {
    ...
    if (skinType == MGUOnOffSkinTypeOn) {
        ...
        self.gradientView1.colors = @[startColor, endColor];
        startColor = [UIColor colorWithRed:242.0/255.0 green:242.0/255.0 blue:242.0/255.0 alpha:1.0];
        endColor = [UIColor colorWithRed:230.0/255.0 green:230.0/255.0 blue:230.0/255.0 alpha:1.0];
        self.gradientViewC1.colors = @[startColor, endColor];
    } else if (skinType == MGUOnOffSkinTypeOff) { // 튀어나온 상태
        UIColor *startColor = [UIColor colorWithRed:143.0/255.0 green:143.0/255.0 blue:143.0/255.0 alpha:1.0];
        UIColor *endColor = [UIColor colorWithRed:114.0/255.0 green:114.0/255.0 blue:114.0/255.0 alpha:1.0];
        self.gradientView1.colors = @[startColor, endColor];
        startColor = [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0];
        endColor = [UIColor colorWithRed:242.0/255.0 green:242.0/255.0 blue:242.0/255.0 alpha:1.0];
        self.gradientViewC1.colors = @[startColor, endColor];
    }
    ...
    if (animated == NO) {
        ...
        if (skinType == MGUOnOffSkinTypeOn) {
            self.centerContainer.layer.borderWidth = 0.66;
