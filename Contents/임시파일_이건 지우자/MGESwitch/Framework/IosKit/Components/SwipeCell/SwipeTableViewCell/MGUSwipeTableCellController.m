...
@interface MGUSwipeTableCellController ()
...
@property (nonatomic, weak, nullable) MGUSwipeTableViewCell *swipeableCell;
...
@implementation MGUSwipeTableCellController
...
static void CommonInit(MGUSwipeTableCellController *self) {
...
- (void)setSwipeOffset:(CGFloat)offset
    ...
    MGUSwipeTableViewCell *swipeableCell = self.swipeableCell;
...
- (void)traitCollectionDidChangeFromPreviousTraitCollrection:(UITraitCollection * _Nullable)previousTraitCollrection
    ...
    if (swipeableCell.state ==     MGUSwipeStateLeft || swipeableCell.state ==     MGUSwipeStateRight) {
        ...
        NSLog(@"targetOffset %f", targetOffset);
...
- (void)handlePan:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        UIView *swipeable = [self.tableView.swipeCells mgrFilterFirst:^BOOL(MGUSwipeTableViewCell * cell) {
        ...
        if ( (translation + self.originalCenter - CGRectGetMidX(swipeableCell.bounds)) * actionsView.orientation > 0.0) {
            ...
            swipeableCell.actionsView.visibleWidth = ABS(CGRectGetMinX(swipeableCell.frame));
        ...
        if (expansionStyle != nil && self.tableView != nil) {
            ...
            if (expanded && !actionsView.expanded && targetOffset > currentOffset) { //! unexpanded => expanded
                ...
                target.center = targetCenter;
//                target.center = CGPointMake(self.originalCenter + [gesture translationInView:gesture.view].x, target.center.y);
                swipeableCell.actionsView.visibleWidth = ABS(CGRectGetMinX(actionsContainerView.frame));
            }
...
//! 놔진 상태에서 가속도가 붙는 방향으로 마무리 처리될 수 있도록 하게 해준다. 열건지 닫을 건지.
- (MGUSwipeState)targetStateForVelocity:(CGPoint)velocity {
    ...
    if (actionsView.orientation == MGUSwipeActionsOrientationLeft) { // 왼쪽이 열렸다.
        return (velocity.x < 0.0 && !actionsView.expanded) ? MGUSwipeStateCenter :     MGUSwipeStateLeft;
    } else if (actionsView.orientation == MGUSwipeActionsOrientationRight) { // 오른쪽이 열렸다.
...
//     self.originalLayoutMargins = self.swipeableCell.layoutMargins; // 드래그 시에 원본 layoutMargins 이 필요하다.
    ...
    if ((orientation == MGUSwipeActionsOrientationLeft && [self.tableView mgrIsRTLLocale] == NO) ||
        (orientation == MGUSwipeActionsOrientationRight && [self.tableView mgrIsRTLLocale] == YES)) {
    ...
    if (configuration.actions == nil || configuration.actions.count <= 0 ||
        self.swipeableCell == nil || self.actionsContainerView == nil || self.tableView == nil) {
        return NO;
    }
    ...
    CGRect visibleTableViewRect = [self.delegate swipeController:self visibleRectForTableView:self.tableView];
    ...
    [[MGUSwipeActionsView alloc] initWithContentEdgeInsets:contentEdgeInsets
                                      ...
                                      safeAreaInsetView:self.tableView
    ...
    [actionsView.heightAnchor constraintEqualToAnchor:self.swipeableCell.heightAnchor].active = YES;
    [actionsView.widthAnchor constraintEqualToAnchor:self.swipeableCell.widthAnchor multiplier:2.0].active = YES; // 길이 2배.
    [actionsView.topAnchor constraintEqualToAnchor:self.swipeableCell.topAnchor].active = YES;
...
//!    [self stopAnimatorIfNeeded];
    ...
    [animator addAnimations:^{
        MGUSwipeTableViewCell *swipeTableViewCell = self.swipeableCell;
        UIView *actionsContainerView = self.actionsContainerView;
        if (swipeTableViewCell == nil || actionsContainerView == nil) {
            return;
        }
        ...
        [swipeTableViewCell layoutIfNeeded];
    }];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer == self.tapGestureRecognizer) {
        ...
        MGUSwipeTableViewCell *swipedCell = nil;
...
- (void)performFillAction:(MGUSwipeAction *)action
    ...
    if (fillOption ==     MGUSwipeCompletionAnimationTypeFillReverse) {
        ...
        [self animateWithDuration:UITableViewDeleteRowDuration
                         toOffset:newCenter
              withInitialVelocity:0.0
