...
- (NSArray <NSLayoutConstraint *>*)mgrPinEdgesToSuperviewCustomMargins:(NSEdgeInsets)customMargins { // 5,5,5,5 이면 안쪽으로 5만큼 파고든다.
    NSCAssert(self.superview, @"Superview 는 nil 이어서는 안된다.");
    self.translatesAutoresizingMaskIntoConstraints = NO;
...
#pragma mark - other
- (NSEdgeInsets)safeAreaInsetsOfSuperview {
...
- (void)mgrRemoveAllConstraints:(BOOL)translatesAutoresizingMaskIntoConstraints {
    NSView *mySuperView = self.superview;
    while (mySuperView != nil) {
        for (NSLayoutConstraint *constraint in mySuperView.constraints) {
            ...
            if ([first isKindOfClass:[NSView class]] && first == self) {
                [mySuperView removeConstraint:constraint];
            }
            if ([second isKindOfClass:[NSView class]] && second == self) {
                [mySuperView removeConstraint:constraint];
            }
        }
