...
@implementation MGROutlineContent
...
+ (instancetype)itemWithTitle:(NSString *)title
          viewControllerClass:(Class)viewControllerClass {
    return [[MGROutlineContent alloc] initWithTitle:title viewControllerClass:viewControllerClass];
}

+ (instancetype)itemWithTitle:(NSString *)title {
    return [[MGROutlineContent alloc] initWithTitle:title viewControllerClass:nil];
}
...
- (id)copyWithZone:(NSZone *)zone {
    MGROutlineContent *item = [[[self class] allocWithZone:zone] init];
    if (item) {
...
- (BOOL)isEqualToContentItem:(MGROutlineContent *)item {
    if (self == item) {
        return YES;
    }
