...
+ (MGRFuture<ObjectType> *)future:(void (NS_NOESCAPE ^)(MGRPubPromiseBlock promise))fulfill;
+ (MGRFuture<ObjectType> *)futureIn:(MGRPubPromiseBlock _Nullable __strong * _Nonnull)promise;
+ (MGRFuture<ObjectType> *)on:(MGRPubQueue)queueType future:(void (NS_NOESCAPE ^)(MGRPubPromiseBlock promise))fulfill;
- (instancetype)initWithData:(ObjectType)data;
@end
@interface MGRFuturistic<__covariant ObjectType> : MGRFuture <ObjectType>
@property (nonatomic, nullable) id data; // Dynamic
- (instancetype)initWithData:(id)data;
- (void)emit:(id)value; // 종료하지 않고 배출만 한다. 지속값 배출에 사용되므로, 완성 자료 배출은 setData:를 이용하자.
@end
...
- (MGRPubTicket *)sinkMain:(MGRPubSuccessBlock)success;
// 내려받기. 성공과 종료를 받는다. 종료는 항상 메인큐로 떨어진다.
- (MGRPubTicket *)sink:(MGRPubSuccessBlock)success finish:(MGRPubFinishBlock)finish;
- (MGRPubTicket *)sinkMain:(MGRPubSuccessBlock)success finish:(MGRPubFinishBlock)finish;
// 내려받기. 성공과 실패를 받는다.
- (MGRPubTicket *)sink:(MGRPubSuccessBlock)success failure:(MGRPubFailureBlock)failure;
- (MGRPubTicket *)sinkMain:(MGRPubSuccessBlock)success failure:(MGRPubFailureBlock)failure;
// 내려받기. 성공, 실패, 종료 모두 받는다. 종료는 항상 메인큐로 떨어진다.
- (MGRPubTicket *)sink:(MGRPubSuccessBlock)success failure:(MGRPubFailureBlock)failure finish:(MGRPubFinishBlock)finish;
- (MGRPubTicket *)sinkMain:(MGRPubSuccessBlock)success failure:(MGRPubFailureBlock)failure finish:(MGRPubFinishBlock)finish;
@end

// 스트림 끝단: 이 지점에서 티켓이 만들어지며, 상위로 거슬러 올라가 파이프를 누적해 반환된다.
@interface MGRPub<ObjectType> (Assign)
// 할당하기. 성공값. ticket을 cancel 하거나 owner 가 nil 이면 종료한다. 종료는 항상 메인큐로 떨어진다.
// 오너와 키패스 및 셀렉터 경로는 동일해야 한다. 키패스 및 셀렉터가 오너에서 실행되기 때문이다.
- (MGRPubTicket *)assignMain:(id)owner keypath:(NSString *)keypath;
- (MGRPubTicket *)assignMain:(id)owner keypath:(NSString *)keypath finish:(MGRPubFinishBlock)finish;;
- (MGRPubTicket *)assignMain:(id)owner selector:(SEL)selector;
- (MGRPubTicket *)assignMain:(id)owner selector:(SEL)selector finish:(MGRPubFinishBlock)finish;;
@end
...
//         [A]
//          │       받은 결과값을 하위에 공유한다.
//       [SHARE]    모든 파이프가 Cold 형식으로 매번 호출되지만
...
- (MGRPub<ObjectType> *)lineMain;
- (MGRPub<ObjectType> *)lineHigh;
- (MGRPub<ObjectType> *)lineLow;
- (MGRPub<ObjectType> *)lineBack;
@end
...
//     [A]-- a -- b -- c -- d --
//           └┐   └┐   └┐   └┐  line 이 지정되어 있으면
//           ┌┘   ┌┘   ┌┘   ┌┘  lineQueue 에서 async 되며
// async    [a]  [b]  [c]  [d]  지정되어 있지 않으면 main 이다.
@interface MGRPub<ObjectType> (Async)
- (MGRPub<ObjectType> *)async;
@end
...
typedef BOOL (^MGRPubMapBoolBlock)(ObjectType value);
...
// 성공값을 받아 이를 판단하여 Bool로 반환. YES 면 @(YES)로, NO 면 @(NO)로 발행한다.
- (MGRPub<MGRBool> *)mapBool:(MGRPubMapBoolBlock)boolBlock;
@end
...
// 직전값과 같으면 무시하고, 다르면 발행한다.
// By는 필터 내에서 isEqual 여부를 평가하며 YES 면 동일. 즉, 내리지 않는다.
// NSNumber 에 실수가 담길 경우 입실론 비교를 해야 하므로 By 를 사용하자.
//    [A]----a----b-----b----b----c----
//           |    |               |
//           a    b               c
@interface MGRPub<ObjectType> (SkipEqual)
typedef BOOL (^MGRPubEqualFilterBlock)(ObjectType prev, ObjectType curr);
- (MGRPub<ObjectType> *)skipEqual;
- (MGRPub<ObjectType> *)skipEqualBy:(MGRPubEqualFilterBlock)filterBlock;
@end


// 직전값의 버퍼를 지정한다. 해당 갯수만큼 히스토리를 갖는다. 0 이상을 넣어야 한다.
// count: 1 이면 기본값이다. @[직전값, 현재값] 으로 넘어간다.
// count 는 버퍼 갯수를 의미한다. 배열 카운트가 아니다.
//    [A]-----a--------------------b---------------c-----------
//            |                    |               |
// count:2   [NSNull, NSNull, a]  [NSNull, a, b]   [a, b, c]
//   └ 버퍼2개. 즉, 배열에 @[이전값, 이전값, 현재값] 이 넘어간다.
@interface MGRPub<ObjectType> (Buffer)
- (MGRPub<NSArray<ObjectType>*> *)buffer; // 버퍼 1개. @[prev, curr]
- (MGRPub<NSArray<ObjectType>*> *)bufferCount:(NSUInteger)count; // @[prev..., curr]
@end
...
// 입력 이벤트 이후 지정된 시간만큼 기다렸다가 다른 입력이 없으면 그 이벤트를 배출한다.
- (MGRPub<ObjectType> *)debounce:(NSTimeInterval)interval;
// 입력 이벤트 배출 이후 지정된 시간동안 늦췄다가 다음 이벤트를 받는다.
- (MGRPub<ObjectType> *)throttle:(NSTimeInterval)interval;
@end


// 매치 여부가 성공이면 @(YES) 를 반환, 아니면 @(NO) 반환
//     [A] ------ a --- b --- c -- d -- e
//         -----------------[YES]
// contains:c     ..........@(YES)
@interface MGRPub<ObjectType> (Match)
typedef BOOL (^MGRPubMatchBlock)(ObjectType value);
// 중간에 성공하면 @(YES) 반환 후 즉시 종료한다. 성공하지 않으면 끝까지 평가한다.
- (MGRPub<MGRBool> *)contains:(MGRPubMatchBlock)evaluator;
// 중간에 실패하면 @(NO) 반환 후 즉시 종료한다. 실패하지 않으면 끝까지 평가한다.
- (MGRPub<MGRBool> *)allSatisfy:(MGRPubMatchBlock)evaluator;
@end
