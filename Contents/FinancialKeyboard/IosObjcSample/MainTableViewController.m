...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"Financial Keyboard";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        if(indexPath.row == 0){
            UIStoryboard *board = [UIStoryboard storyboardWithName:@"ViewControllerB"
                                                            bundle:bundle];
            UIViewController *result = [board instantiateViewControllerWithIdentifier:@"ViewControllerB"];
            [self.navigationController pushViewController:result animated:YES];
        } else if(indexPath.row == 1){
            UIStoryboard *board = [UIStoryboard storyboardWithName:@"ViewControllerC"
                                                            bundle:bundle];
            UIViewController *result = [board instantiateViewControllerWithIdentifier:@"ViewControllerC"];
            [self.navigationController pushViewController:result animated:YES];
        }
    }
}
